...
/**
 * 挙兵の基礎確率。毎年、王ひとりごとに判定する。
 *
 * 0.0035 に置いていたのは、諸王が平均0.8人しか立っていなかった頃の値で、
 * 名簿が埋まる（平均1.6人）ように直したら挙兵が倍になり、
 * 中級の存続が54%から31%へ、藩王の即位が19%から57%へ振れた。
 * **一年あたりに天下で起きる挙兵の頻度**が調整済みの量なので、
 * 王の数が増えたぶんだけ一人あたりの確率を下げる
 */
export const PRINCE_REVOLT_BASE = 0.0014;
/** 野心1あたり、挙兵の確率に乗る倍率 */
 ...
 */
export const PRINCE_MARCH_PROBABILITY = 0.22;
/** 都を衝くのに要る兵の比。守りをこれだけ上回らなければ攻め上らない */
export const PRINCE_MARCH_MARGIN = 1.25;
/**
 * 王が即位したときに戻る天命。**禅譲の閾値をわずかに超えるだけにする。**
 *
 * 44 に置いていたときは、簒奪が朝廷を立て直す手当てとして働き、
 * 傾いた王朝が藩王の即位で息を吹き返して禅譲が起きなくなった
 * （王朝の交替が1局あたり3.5回から2.4回へ落ちた）。
 * 趙王倫の簒奪は晋を立て直しはしなかった — 位に即いた王が受け取るのは
 * 「まだ潰えていない」という程度の正統性でしかない
 */
export const PRINCE_ENTHRONE_MANDATE = 36;
/** 王が即位したときに失う士族の支持 */
...
/**
 * 都督の能力1あたり、簒奪の判定が始まる天命の閾値を押し上げる幅。
 *
 * **天命が尽きたときにしか位が渡らない作りにしていたときは、
 * 王朝の交替が1局に1.4回しか起きなかった。** 291年から589年までに
 * 南朝は晋・宋・斉・梁・陳と四度替わっている。しかも劉裕が晋から受けたのは
 * 晋が衰えていたからではない — **北伐に勝ち続けた将が、勝ったからこそ
 * 位を受けた。** 蕭道成も陳霸先も同じである。
 * 有能な都督を置くこと自体が危うい、という第一の主題がここにも要る
 */
export const USURPATION_MARSHAL_THRESHOLD_PER_POINT = 4;
/**
 ...
 * 一族の人数の上限。これ以上は生まれない。
 *
 * 6人にしていたときは、成人の一族が平均1.5人しかおらず、
 * そのうち一人は太子なので**封じられる者がほとんど残らなかった**
 * （312年以降の44%の年で、封じられる成人が一人もいない）。
 * 諸王が枯れれば「宗室に兵を預ければ中央が倒れる」という主題が立たない。
 * 宋の文帝に19子、梁の武帝に8子がいたように、帝の子は多い
 */
export const DYNASTY_MEMBER_MAX = 9;
